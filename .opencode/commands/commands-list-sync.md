...
This command must not modify wiki pages, raw sources, or other project files. Step 3 may write only `commands-list.md` and `.commands-manifest.txt`, its internal cache files.

## Step 1: Cache Check

Determine every command's modification time cross-platform. Try `stat -f '%N:%m' <file>` (macOS/BSD), then fall back to `stat -c '%n:%Y' <file>` (Linux/GNU):

```
find .opencode/commands -maxdepth 1 -name '*.md' ! -name 'commands-list-sync.md' -exec stat -f '%N:%m' {} \; | sort
```

If that fails, use:

```
find .opencode/commands -maxdepth 1 -name '*.md' ! -name 'commands-list-sync.md' -exec stat -c '%n:%Y' {} \; | sort
```

Compare the output with `.opencode/commands/.commands-manifest.txt`.

- **Exact match:** print `commands-list.md` as-is without rereading command files, then stop.
...
If Step 2 regenerated the list, show the proposed `commands-list.md` changes and request explicit confirmation before writing it and the current manifest to `.opencode/commands/.commands-manifest.txt`.

## Commit Suggestion

If Steps 2 and 3 regenerated the cache, end with: `Manual Git command: git add -A && git commit -m "chore: sync command list"`. On a cache hit, no commit is needed. Do not run it.
